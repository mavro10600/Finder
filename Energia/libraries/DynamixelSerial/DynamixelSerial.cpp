...
#define sendData(args)  (iostream->write(args))    // Write Over Serial
#define availableData() (iostream->available())    // Check Serial Data Available
#define readData()      (iostream->read())         // Read Serial Data
#define peekData()      (iostream->peek())         // Peek Serial Data
#define beginCom(args)  (iostream->begin(args))    // Begin Serial Comunication
#define endCom()        (iostream->end())          // End Serial Comunication
// Macro for Timing
...
DynamixelClass::DynamixelClass(HardwareSerial* io) {
	iostream = io;
	Direction_Pin = -1;
}

DynamixelClass::DynamixelClass(HardwareSerial* io, uint8_t directionPin) {
	iostream = io;
...
void DynamixelClass::begin(long baud, unsigned char directionPin) {
	this->setDirPin(directionPin);
	beginCom(baud);
}

void DynamixelClass::begin(long baud) {
	this->begin(baud, Direction_Pin);
}
...
int DynamixelClass::move(unsigned char ID, int Position) {
	...
	Position_L = Position;
	Checksum = (~(ID + AX_GOAL_LENGTH + AX_WRITE_DATA + AX_GOAL_POSITION_L
...
int DynamixelClass::setEndless(unsigned char ID, bool Status) {
	if (Status) {
		...
		Checksum =
				(~(ID + AX_GOAL_LENGTH + AX_WRITE_DATA + AX_CCW_ANGLE_LIMIT_L))
						& 0xFF;
...
int DynamixelClass::moveRW(unsigned char ID, int Position) {
	...
	Position_H = Position >> 8;           // 16 bits - 2 x 8 bits variables
	Position_L = Position;
	Checksum = (~(ID + AX_GOAL_LENGTH + AX_REG_WRITE + AX_GOAL_POSITION_L
			+ Position_L + Position_H)) & 0xFF;
...
int DynamixelClass::torqueStatus(unsigned char ID, bool Status) {
	Checksum = (~(ID + AX_TORQUE_LENGTH + AX_WRITE_DATA + AX_TORQUE_ENABLE
			+ Status)) & 0xFF;
...
int DynamixelClass::readTemperature(unsigned char ID) {
	Checksum = (~(ID + AX_TEM_LENGTH + AX_READ_DATA + AX_PRESENT_TEMPERATURE
			+ AX_BYTE_READ)) & 0xFF;
...
int DynamixelClass::readVoltage(unsigned char ID) {
	Checksum = (~(ID + AX_VOLT_LENGTH + AX_READ_DATA + AX_PRESENT_VOLTAGE
			+ AX_BYTE_READ)) & 0xFF;
...
int DynamixelClass::setTempLimit(unsigned char ID, unsigned char Temperature) {
	Checksum = (~(ID + AX_TL_LENGTH + AX_WRITE_DATA + AX_LIMIT_TEMPERATURE
			+ Temperature)) & 0xFF;
...
int DynamixelClass::setVoltageLimit(unsigned char ID, unsigned char DVoltage,
		unsigned char UVoltage) {
	Checksum = (~(ID + AX_VL_LENGTH + AX_WRITE_DATA + AX_DOWN_LIMIT_VOLTAGE
			+ DVoltage + UVoltage)) & 0xFF;
...
int DynamixelClass::setAngleLimit(unsigned char ID, int CWLimit, int CCWLimit) {
	...
	CCW_H = CCWLimit >> 8;
	CCW_L = CCWLimit;
	Checksum = (~(ID + AX_VL_LENGTH + AX_WRITE_DATA + AX_CW_ANGLE_LIMIT_L + CW_H
			+ CW_L + AX_CCW_ANGLE_LIMIT_L + CCW_H + CCW_L)) & 0xFF;
...
int DynamixelClass::setSRL(unsigned char ID, unsigned char SRL) {
	Checksum = (~(ID + AX_SRL_LENGTH + AX_WRITE_DATA + AX_RETURN_LEVEL + SRL))
			& 0xFF;
...
int DynamixelClass::setRDT(unsigned char ID, unsigned char RDT) {
	Checksum = (~(ID + AX_RDT_LENGTH + AX_WRITE_DATA + AX_RETURN_DELAY_TIME
			+ (RDT / 2))) & 0xFF;
...
int DynamixelClass::setLEDAlarm(unsigned char ID, unsigned char LEDAlarm) {
	Checksum = (~(ID + AX_LEDALARM_LENGTH + AX_WRITE_DATA + AX_ALARM_LED
			+ LEDAlarm)) & 0xFF;
...
int DynamixelClass::setShutdownAlarm(unsigned char ID, unsigned char SALARM) {
	Checksum = (~(ID + AX_SALARM_LENGTH + AX_ALARM_SHUTDOWN + AX_ALARM_LED
			+ SALARM)) & 0xFF;
...
int DynamixelClass::setCMargin(unsigned char ID, unsigned char CWCMargin,
		unsigned char CCWCMargin) {
	Checksum = (~(ID + AX_CM_LENGTH + AX_WRITE_DATA + AX_CW_COMPLIANCE_MARGIN
			+ CWCMargin + AX_CCW_COMPLIANCE_MARGIN + CCWCMargin)) & 0xFF;
...
int DynamixelClass::setCSlope(unsigned char ID, unsigned char CWCSlope,
		unsigned char CCWCSlope) {
	Checksum = (~(ID + AX_CS_LENGTH + AX_WRITE_DATA + AX_CW_COMPLIANCE_SLOPE
			+ CWCSlope + AX_CCW_COMPLIANCE_SLOPE + CCWCSlope)) & 0xFF;
...
int DynamixelClass::setPunch(unsigned char ID, int Punch) {
	...
	Punch_L = Punch;
	Checksum = (~(ID + AX_PUNCH_LENGTH + AX_WRITE_DATA + AX_PUNCH_L + Punch_L
			+ Punch_H)) & 0xFF;
...
int DynamixelClass::moving(unsigned char ID) {
	Checksum = (~(ID + AX_MOVING_LENGTH + AX_READ_DATA + AX_MOVING
			+ AX_BYTE_READ)) & 0xFF;
...
int DynamixelClass::RWStatus(unsigned char ID) {
	Checksum = (~(ID + AX_RWS_LENGTH + AX_READ_DATA + AX_REGISTERED_INSTRUCTION
			+ AX_BYTE_READ)) & 0xFF;
...
int DynamixelClass::readSpeed(unsigned char ID) {
	Checksum = (~(ID + AX_POS_LENGTH + AX_READ_DATA + AX_PRESENT_SPEED_L
			+ AX_BYTE_READ_POS)) & 0xFF;
...
int DynamixelClass::readLoad(unsigned char ID) {
	Checksum = (~(ID + AX_POS_LENGTH + AX_READ_DATA + AX_PRESENT_LOAD_L
			+ AX_BYTE_READ_POS)) & 0xFF;
